...
def prepare_sdm(**settings):
    global dftmodes
    sdm = pm.spectral_density_model(w = np.linspace(1e-5,0.5,1000), time = np.linspace(0,500,1000), E = settings.get('E',np.linspace(-0.5,0.5,500)))
...
sts = {'Vc': 57.329551960119417, 'alpha_source': 0.48694930170632039, 'alpha_gate': 0.0051023140720955094} # these values are obtained from the manual_fit_Vc() and manual_fit_alpha() functions
params = {'VS': 5e-4, 'VD': 5e-6, 'lmbd': 0.2, 'wc': 0.025, 'T': 77, 'alpha_source': 0.4863911384199851, 'spin': 0.0} # initial guesses for parameters
dat.ps(**sts)
dat.resample(128,128)
dat.correct_offset(zero=0.01)
trace = dat.resonance_bias_trace()
trace['Isd']*=1e9
params,r2,fit = trace.fit( # this is the fitting function..so we fit the calculate_IV and return the parameters
    calculate_IV,
    p0 = params,
    bounds = {
        'VS': [1e-6,1e-1],
    ...
    fix=['T','alpha_source','spin','wc'], # fix those that you know, here we fit: 'VS','VD','lmbd'
    followprogress=True,
    return_dict=True
)
print(params)
...
fitsd['Isd'] = np.reshape(stabdiag(fitsd['Vsd'].values, fitsd['Vg'].values, **params),fitsd['Vsd'].values.shape)
fig = Figure()
