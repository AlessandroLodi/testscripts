...
def move_to_center(g):
    ...
    g = g.translate(
        [cell_center[0] - geom_center[0], cell_center[1] - geom_center[1], 0]
    )
    plot(g)
    plt.axis("equal")
...
def display(g, aid=False, sc=True, figsize=(10, 5)):
    ...
    if length[0] > length[1]:  # x is larger
        plt.xlim(minxyz[0] - 2, maxxyz[0] + 2)
    else:
        plt.ylim(minxyz[1] - 2, maxxyz[1] + 2)


def connect(g1, g2, a1, a2):
    """
    Connect two geometries using a horizontal single C-C bond.
    """

    Ra1 = g1.Rij(0, a1)
    Ra2 = g2.Rij(a2, 0)
    bond = np.array([1.42, 0, 0])
    displ = Ra1 + bond + Ra2
    g = g1.add(g2, displ)
...
def pdos(H, Erange=(-20, 20), figsize=(6, 4)):
    ...
    def wrap(PDOS):
        ...
        return np.stack(
            (pdos_s, pdos_pxy, pdos_pz, pdos_d, pdos_hs, pdos_hpxy, pdos_hpz)
        )
...
def fat_bands(H, Erange=(-20, 20), figsize=(10, 8)):
    ...
    for i, e in enumerate(eig):
        ...
        # pz
        plt.fill_between(
            linear_k, e + (s + pxy), e + (s + pxy + pz), color="C2", alpha=0.5
        )
        plt.fill_between(
            linear_k, e - (s + pxy + pz), e - (s + pxy), color="C2", alpha=0.5
        )
        # d
        plt.fill_between(
            linear_k, e + (s + pxy + pz), e + (s + pxy + pz + d), color="C3", alpha=0.5
        )
        plt.fill_between(
            linear_k, e - (s + pxy + pz + d), e - (s + pxy + pz), color="C3", alpha=0.5
        )
        # hs
        plt.fill_between(
            linear_k,
    ...
    for i, e_all_k in enumerate(eig):
        ...
        if np.any(pz_abs / dE > 0.5):
            ...
            for i in range(len(klist)):
                ...
                # pz
                plt.fill_between(
                    k_pz, e + (s + pxy), e + (s + pxy + pz), color="C2", alpha=0.5
                )
                plt.fill_between(
                    k_pz, e - (s + pxy + pz), e - (s + pxy), color="C2", alpha=0.5
                )
                # d
                plt.fill_between(
                    k_pz,
...
@timer
def ldos(H, E=0.0, Ewidth=0.1, height=1.0, mesh=0.1, figsize=(15, 5), colorbar=False):
    """
