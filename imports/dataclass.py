...
class Figure:
        ...
        if subplot["type"] == "color":
            for dataset in subplot.datasets:
                ...
                if subplot["cbar"] != "None":
                    ...
                    subplot["cbar"] = plt.colorbar(format="%.2g", shrink=0.9)
    ...
    def visualise(
        ...
        subplots = np.empty(
            ...
            dtype=matplotlib.axes.SubplotBase,
        )
...
class Data:
    ...
    def __getitem__(self, item):
        ...
        if type(item) is slice or np.issubdtype(item, int):
            if len(self._data) != 1:
                raise RuntimeError(f"Incorrect indexing, cannot find a single value for index {item}.")
...
if __name__ == '__main__':
    ...
    dat = r'C:\Users\albus\OneDrive\Desktop\model fitting james thomas and bart\170822_TL_300_6_TL_IVsVg-raw_g14.dat'
    d = Stability_Diagram.load_from_file(dat, cyclic_method='average')
    d.plot_settings(T=77)