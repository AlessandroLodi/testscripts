...
class spectral_density_model:
    ...
    def add_fake_damped_mode(self,height,width,omega):
        self.specden = True
        num = height * (self.w**3 / omega**3) * np.exp(-self.w/(width*omega)) * np.exp(-omega/(width*omega))
    ...
    def add_damped_mode(self,omega,n=0.02,lambd=5e-3,scale=1):
        ...
        J2 += (self.w - omega + n * lambd - n * self.w * np.exp(-self.w / lambd) * Ei) ** 2
        J /= J2
        self.J+=J
    ...
    def calculate_rate_constants(self,Y=0,T=77):
        ...
        for index,t in enumerate(time):
            ...
            for mode in self.modes:
                omega, g = mode
                D1 = (g**2 / omega**2) * ((np.cos(omega*t)-1) / np.tanh(betaph*omega/2) - 1j*np.sin(omega*t))
    ...
    def calculate_IV(self,Vbs, VS, VD, alpha_source, T, spin=0.5):
        ...
        eps0 = 0
        eps_m = eps0 - alpha_source * Vbs  # eps is raised by Vs and Vg by their corresponding alphas. Vd = 0
        ...
        return (1.6e-3 / 6.582)*(y_ox_S * y_red_D - y_red_S * y_ox_D) / (y_ox_S + y_ox_D + y_red_S + y_red_D)

    def calculate_IVsVg(self, Vbs, Vgs, VS, VD, alpha_source, alpha_gate, Vc, T, spin=0.5):
        # try: _ = self.k_ox
        # except: self.calculate_rate_constants(VS,VD)
        kb = 8.617e-5  # boltzmann constant in eV.K-1
        ...
        # prepare 3D arrays of epsilon (mol level) and E for integration
        eps0 = alpha_gate * Vc
        eps_m = eps0 - alpha_gate * Vgs - alpha_source * Vbs  # eps is lowered by Vs and Vg by their corresponding alphas. Vd = 0
        ...
        if spin == 0:
            ...
            y_ox_D *= 2
        # calculate the current
        I = np.nan_to_num((1.6e-3 / 6.582)*(y_ox_S * y_red_D - y_red_S * y_ox_D) / (y_ox_S + y_ox_D + y_red_S + y_red_D))
    ...
    def marcus_jortner_rate_constants(self, lmbd0, S, w0, T):
        ...
        for v in range(100):
            self.k_ox += pf * np.exp(-S) * ((S ** v) / (np.math.factorial(v))) * np.exp(-((lmbd0 + v * w0 + E) ** 2 / den))
            self.k_red += pf * np.exp(-S) * ((S ** v) / (np.math.factorial(v))) * np.exp(-((lmbd0 + v * w0 - E) ** 2 / den))
        self.E = E
...
class physics_models:
    ...
    @staticmethod
    def doubledotSD(Vbs, Vg, lam, VS, VD, alpha_source1, alpha_source2, alpha_gate, T, X = 0, eps0_1 = 0, eps0_2 = 0, HAB = 1e-3, spin=0.5):
        print('>> Fitting IV trace... lam: {}, VS: {}, VD: {}, X: {}'.format(lam, VS, VD, X))
        ...
        # prepare 3D arrays of epsilon (mol level) and E for integration
        eps_m1 = eps0_1 - alpha_source1 * Vbs - alpha_gate * Vg  # eps is raised by Vs and Vg by their corresponding alphas. Vd = 0
        eps_m2 = eps0_2 - alpha_source2 * Vbs - alpha_gate * Vg  # eps is raised by Vs and Vg by their corresponding alphas. Vd = 0

        k_12 = (np.pi * betaph / lam) ** 0.5 * np.exp(-(lam - (eps_m1 - eps_m2)) ** 2 / (4 * lam / betaph + X / (3 * kb * T)))
        k_21 = (np.pi * betaph / lam) ** 0.5 * np.exp(-(lam - (eps_m2 - eps_m1)) ** 2 / (4 * lam / betaph + X / (3 * kb * T)))
        ...
        k_red_S = (np.pi * betaph / lam) ** 0.5 * np.exp(-(lam - (E1 - eps_m1)) ** 2 / (4 * lam / betaph + X / (3 * kb * T)))
        k_ox_S = (np.pi * betaph / lam) ** 0.5 * np.exp(-(lam + (E1 - eps_m1)) ** 2 / (4 * lam / betaph + X / (3 * kb * T)))

        k_red_D = (np.pi * betaph / lam) ** 0.5 * np.exp(-(lam - (E2 - eps_m2)) ** 2 / (4 * lam / betaph + X / (3 * kb * T)))
        k_ox_D = (np.pi * betaph / lam) ** 0.5 * np.exp(-(lam + (E2 - eps_m2)) ** 2 / (4 * lam / betaph + X / (3 * kb * T)))
        ...
        I = (y_21*y_red_D*y_ox_S - y_12*y_ox_D*y_red_S) / (y_ox_S*y_ox_D + y_12*y_ox_D+ y_21*y_ox_S + y_red_D*(y_ox_S + y_12+ y_21) + y_red_S*(y_ox_D + y_12 +y_21))

        return I.flatten() * 1.6e-3 / 6.582

    @staticmethod
    def doubledot(Vbs, lam, VS, VD, alpha_source1, alpha_source2, T, X = 0, eps0_1 = 0, eps0_2 = 0, HAB = 1e-3, spin=0.5):
        print('>> Fitting IV trace... lam: {}, VS: {}, VD: {}, X: {}'.format(lam, VS, VD, X))
        ...
        # prepare 3D arrays of epsilon (mol level) and E for integration
        eps_m1 = eps0_1 - alpha_source1 * Vbs  # eps is raised by Vs and Vg by their corresponding alphas. Vd = 0
        eps_m2 = eps0_2 - alpha_source2 * Vbs  # eps is raised by Vs and Vg by their corresponding alphas. Vd = 0
    ...
    def improved_marcus(Vbs, lam, VS, VD, alpha_source, T, X, spin=0.5):
        ...
        # calculate the hopping rate constants
        k_red = (np.pi * betaph / lam) ** 0.5 * np.exp(-(lam - (E - eps_m)) ** 2 / (4 * lam / betaph + X / (3 * kb * T)))
        k_ox = (np.pi * betaph / lam) ** 0.5 * np.exp(-(lam + (E - eps_m)) ** 2 / (4 * lam / betaph + X / (3 * kb * T)))
        ...
        return I.flatten() * 1.6e-3 / 6.582
    @staticmethod
    def stabdiag_improved_marcus(data, lam, VS, VD, Vc, alpha_source, alpha_gate, T, X, spin=0.5):
        ...
        # prepare 3D arrays of epsilon (mol level) and E for integration
        eps0 = alpha_gate * Vc
        eps_m = eps0 - alpha_gate * Vg - alpha_source * Vbs  # eps is lowered by Vs and Vg by their corresponding alphas. Vd = 0
        E = np.repeat(E[np.newaxis, :], eps_m.shape[1], axis=0)
    ...
    def stabdiag_marcus(data, lam, VS, VD, Vc, alpha_source, alpha_gate, T):
        return physics_models.stabdiag_improved_marcus(data, lam, VS, VD, Vc, alpha_source, alpha_gate, T, 0)
        ...
        for i in range(len(x)):
            ...
            for j in range(len(Ex)):
                if Ex[j] > phi_R:
                    ...
                    F.append(d * (2 / 3) * ((phi_R - Ex[j]) ** (3 / 2) - (phi_L - Ex[j]) ** (3 / 2)) / (phi_R - phi_L))
    ...
    def stabdiag(data, prefactor, Vc, alpha_gate, alpha_source, T):
        ...
        Vs = data[1]
        Vg = data[0]
        mu = alpha_gate * Vc - alpha_gate * Vg - alpha_source * Vs  # == mu0 - alpha_gate * Vg - alpha_source * Vs
        fd = 1 / (1 + np.exp(mu / (kb * T)))
        fs = 1 / (1 + np.exp((mu + Vs) / (kb * T)))
        return prefactor * (fd - fs)

    @staticmethod
    def curved_improved_marcus(data, lam, VS, VD, Vc, alpha_source, alpha_gate, alpha_J, J, T, X, spin=0.5):
        # kb = 8.6173303e-5  # eV.K-1
        Vbs = data[1]
        Vg = data[0]
        ...
        # prepare 3D arrays of epsilon (mol level) and E for integration
        eps0 = alpha_gate * Vc
        eps_m = eps0 - alpha_gate * Vg - alpha_source * Vbs - np.sqrt((alpha_J*Vbs)**2+4*J**2)/2  # eps is lowered by Vs and Vg by their corresponding alphas. Vd = 0
        E = np.repeat(E[np.newaxis, :], eps_m.shape[1], axis=0)
        E = np.repeat(E[np.newaxis, :, :], eps_m.shape[0], axis=0)
        eps_m = np.repeat(eps_m[:, :, np.newaxis], levels, axis=2)
        Vbs = np.repeat(Vbs[:, :, np.newaxis], levels, axis=2)
    ...
    def curved_stabdiag(data, prefactor, Vc, alpha_gate, alpha_source, alpha_J, T, J):
        ...
        Vs = data[1]
        Vg = data[0]
        mu = alpha_gate * Vc - alpha_gate * Vg - alpha_source * Vs - np.sqrt((alpha_J*Vs)**2+4*J**2)/2
        fd = 1 / (1 + np.exp(mu / (kb * T)))
        fs = 1 / (1 + np.exp((mu + Vs) / (kb * T)))
        return prefactor * (fd - fs)