...
for vc in vcs:  # loop over the peaks values
    ...
    fitGsd = physics_models.thermal_broadening(
        vg, **params
    )  # use the optimised parameters and the gate voltage array to generate the Gsd fit values. First argument has to be Vg.
...
# visualise, or save
fig.visualise(fig_dir + f"/fit_gatetrace_python_{int(T)}_K")
...
# change directory
df_gsd.to_csv(path_or_buf=fig_dir +
              f"/zbgt_{int(T)}K.csv", sep=",", index=False)
# writer.save()
